...
sudo -S yum install -y mysql-community-server wget
...
cd /tmp
wget http://10.200.3.106:8888/server/create_alfresco_db.sql -O /tmp/create_alfresco_db.sql
wget http://10.200.3.106:8888/server/grant_alfresco_user.sql -O /tmp/grant_alfresco_user.sql
...
sudo -S cd /tmp
sudo -S wget http://10.200.3.106:8888/server/alfresco-community-4.0.d-installer-linux-x64.bin -O /tmp/alfresco-community-4.0.d-installer-linux-x64.bin
...
cd /home/alfresco
wget http://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-java-5.1.38.tar.gz -O /home/alfresco/mysql-connector-java-5.1.38.tar.gz
tar zxvf /home/alfresco/mysql-connector-java-5.1.38.tar.gz mysql-connector-java-5.1.38/mysql-connector-java-5.1.38-bin.jar
...
wget http://10.200.3.106:8888/server/alfresco.service -O /tmp/alfresco.service
sudo -S cp /tmp/alfresco.service /usr/lib/systemd/system/alfresco.service
sudo -S systemctl enable alfresco.service
